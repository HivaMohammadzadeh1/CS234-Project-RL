#!/usr/bin/env python3
"""
DPO Training with X-CLIP Video Encoder

This version uses X-CLIP (video-aware CLIP) instead of standard CLIP,
which better captures temporal dynamics for video preference learning.

Key improvement: X-CLIP processes videos as sequences rather than
averaging independent frame features.
"""
...
# ─────────────────────────────────────────────────────────────
#  Video Feature Extraction with X-CLIP
# ─────────────────────────────────────────────────────────────

def load_xclip_encoder(device):
    """Load X-CLIP video encoder model."""
    try:
        from transformers import XCLIPVisionModel, XCLIPProcessor
        print("Loading X-CLIP video encoder...")
        ...
        print("ERROR: X-CLIP not available. Install with: pip install transformers")
        raise


def sample_video_frames(video_path, n_frames=8, img_size=224):
    """Sample uniformly-spaced frames from video."""
...
def encode_video_xclip(video_path, model, processor, n_frames=8):
    """Encode video using X-CLIP.

    Returns:
        video_feat: (feat_dim,) video embedding
    """
    frames = sample_video_frames(video_path, n_frames=n_frames)

    # Process with X-CLIP (handles temporal modeling internally)
    inputs = processor(images=frames, return_tensors="pt")
    pixel_values = inputs["pixel_values"].unsqueeze(0).to(model.device)

    # Get video features (X-CLIP processes as video, not individual frames)
...
def extract_xclip_features(videos_dir, video_names, model, processor, n_frames=8):
    """Extract X-CLIP features for all videos."""
    video_features = {}
    missing = []

    print(f"Extracting X-CLIP features for {len(video_names)} videos ({n_frames} frames each)...")

    for i, vname in enumerate(sorted(video_names)):
        vpath = os.path.join(videos_dir, vname)
        if os.path.exists(vpath):
            try:
                feat = encode_video_xclip(vpath, model, processor, n_frames)
...
def stratified_split(comps, val_ratio=0.2, seed=42):
    """Split with group stratification."""
...
# ─────────────────────────────────────────────────────────────
#  Models (same as before, but with X-CLIP features)
# ─────────────────────────────────────────────────────────────
...
class DPOModel(nn.Module):
    """DPO with pretrained BT reference."""
...
# ─────────────────────────────────────────────────────────────
#  Training (same structure as original)
# ─────────────────────────────────────────────────────────────

def train_bt(train_loader, val_loader, feat_dim, n_prompts, args):
    print("=" * 60)
    print("  Bradley-Terry Reward Model (X-CLIP Features)")
    print("=" * 60)
    ...
    torch.save(best_state, os.path.join(args.output_dir, "best_reward_model_xclip.pt"))
...
def train_dpo(train_loader, val_loader, feat_dim, n_prompts, args, beta=None, pretrained_bt_path=None):
    ...
    print("=" * 60)
    print(f"  DPO Model (X-CLIP, beta={beta}, KL={args.kl_penalty})")
    print("=" * 60)
    ...
    model.load_state_dict(best_state)
    torch.save(best_state, os.path.join(args.output_dir, f"best_dpo_xclip_beta{beta}.pt"))
    print(f"  Best val accuracy: {best_val_acc:.4f}  ({time.time() - t0:.1f}s)")
    return model, hist, best_val_acc
...
def parse_args():
    ap = argparse.ArgumentParser(description="DPO with X-CLIP video features")
    ...
    ap.add_argument("--n_frames", type=int, default=8, help="Frames per video")
    ...
    ap.add_argument("--output_dir", type=str, default="dpo_output_xclip")
...
def main():
    ...
    # Load X-CLIP
    xclip_model, xclip_processor = load_xclip_encoder(args.device)

    # Extract features
    cached_features = os.path.join(args.output_dir, "xclip_video_features.pt")
    if os.path.exists(cached_features):
        print("Loading cached X-CLIP features...")
        ...
        video_features = extract_xclip_features(
            args.videos_dir, needed, xclip_model, xclip_processor, args.n_frames
        )
        feat_dim = next(iter(video_features.values())).shape[0]
    ...
    del xclip_model, xclip_processor
    if args.device == "cuda":
    ...
    print(f"      X-CLIP feature dim = {feat_dim}")
    ...
    if args.method in ("dpo", "both"):
        bt_path = os.path.join(args.output_dir, "best_reward_model_xclip.pt")
    ...
    # Summary
    print("\n" + "=" * 60)
    print("  Final Results (X-CLIP Features)")
    print("=" * 60)
